...
void ShieldSpot::update(float percent) {
	float scale = size * (SpaceObject::sys->t - start);
	if (SpaceObject::sys->t - start - 1.0f > 0) {
		done = true;
		return;
	}
	glColor4f(0.5 - 0.5 * percent, 0.5 + 0.5 * percent, 0.5 + 0.5 * percent, 1 + start - SpaceObject::sys->t);
